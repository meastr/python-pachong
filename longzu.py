...
def getNocvelContent():
    html = urllib.request.urlopen("http://www.duoben.net/book/11893/").read()
    html = html.decode("gbk")
    # print(html)
    # dir()
    # <li><a href="http://www.quanshuwang.cn/book/0/742/238294.html" title="第一章 初临异世，共4788字">第一章 初临异世</a></li>
    # .*? 匹配所有
    # <dd> <a href = "/book/11893/4101852.html" > 第4章全民公敌3 </a> </dd>
    # 2.获取章节的超链接
    # reg = r'<dd><a href="(.*?)" title=".*?">(.*?)</a></dd>'  # 利用正则表达式 匹配内容
    # < dd > < a  href = "/book/11893/4101850.html" > 第2章全民公敌1 < / a > < / dd >
    reg = r'<dd><a href="http://www.duoben.net/(.*?)">(.*?)</a></dd>'  # 利用正则表达式 匹配内容
    # 目的是增加效率
    reg = re.compile(reg)
